...
export class ActionsService {
    ...
    private isActionAvailable(actionName: string, player: any, table: any): boolean {
        // Si le joueur n'est pas le joueur actuel, aucune action n'est disponible
        if (!player.isCurrentPlayer) {
            return false;
        }
        
        // Si le joueur a déjà fold, aucune action n'est disponible
        if (player.hasFolded) {
            return false;
        }
        ...
        switch (actionName) {
            case 'fold':
                // Toujours disponible sauf si le joueur peut checker
                return currentBet > playerBet || currentBet > 0;
            
            case 'check':
                // Disponible seulement si le joueur a déjà misé le montant actuel
                // ou s'il n'y a pas de mise en cours
                return currentBet === playerBet;
            
            case 'call':
                // Disponible seulement s'il y a une mise à suivre
                // et que le joueur a assez de jetons
                return currentBet > playerBet && player.chips >= (currentBet - playerBet);
            
            case 'raise':
                // Disponible si le joueur a assez de jetons pour suivre et augmenter
