...
export class GameLogicService {
    ...
    startNextRound(table: any): void {
        ...
        // Réinitialiser le tour et les mises
        table.turn = 1;
        table.currentBet = table.bigBlind;
        table.pot = 0;
        
        // Vider les cartes communes
        table.river = [];
        
        // Réinitialiser complètement l'état des joueurs
        table.players.forEach((player: any) => {
            // Réinitialiser les cartes
            player.hand = [];
            
            // Réinitialiser les mises et statuts
            player.currentBet = 0;
            player.hasFolded = false;
            player.hasPlayed = false;
            ...
            // Conserver les informations du joueur (id, name, chips, tableId, position)
            // et son type (isAI, isHuman)
            
            // Les positions (dealer, blinds, etc.) seront définies par rotateDealer
        });
        
        // Faire tourner le dealer et assigner les blinds
        this.rotateDealer(table);
