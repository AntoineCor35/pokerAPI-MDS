...
export class GameLogicService {
    ...
    private evaluateEndTurn(table: any): any {
        ...
        // Réinitialiser le currentBet de la table à 0 à chaque nouveau tour
        // (les joueurs devront miser à nouveau)
        table.currentBet = 0;
        
        // Réinitialiser les statuts des joueurs pour le nouveau tour
        table.players.forEach((player: any) => {
            // Réinitialiser uniquement pour les joueurs encore actifs
            if (!player.hasFolded) {
                // Réinitialiser les indicateurs d'action
                player.hasPlayed = false;
                player.hasAlreadyRaise = false;
                
                // Réinitialiser la mise courante du joueur pour le nouveau tour
                player.currentBet = 0;
            }
        });
        
        // Distribuer les cartes communes selon le tour actuel
        switch (table.turn) {
            case 2:
                // Le flop (3 premières cartes)
